...
**Контейнеры:** 51 (39 активных) | **БД:** 7 PostgreSQL | **Redis:** 4 | **Домены:** 50+
...
## 🔍 База данных `reports` — Детали

**Назначение:** Временное хранилище для создания аналитических отчётов  
**Статус:** Пустая (инициализирована)  
**Размер:** 8.5 MB

### Функции:
- **Сбор данных** из разных баз (billing, school, support, manyclubs, sudnik)
- **Временные таблицы** для агрегации и трансформации данных
- **Объединение данных** из множественных источников
- **Формирование отчётов** по запросу через AI ассистента
- **Аналитика** по всем проектам экосистемы

### Структура:
Текущие таблицы: **0** (база создаётся динамически под задачи)

### Типичные сценарии использования:
1. Сбор финансовых данных из `billing` + `alex1` для общего отчёта
2. Агрегация пользовательской активности из `school` + `support`
3. Кросс-проектная аналитика по всем базам
4. Временное хранение для построения дашбордов
...
# Подключение
psql -h localhost -p 5433 -U teleeng -d reports
```

**Примечание:** База очищается периодически после завершения работы с отчётами.
...
7. **База `reports`** используется для временных аналитических отчётов и автоматически очищается после использования
